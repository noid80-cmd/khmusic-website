...
- 자료에 없는 상황을 지어내 쓰지 마세요. "실기장에서 곡을 더 요청하는 경우가 있습니다"처럼 근거를 댈 수 없는 문장이 여기 해당합니다. 아래 "알아두어야 할 배경"은 무엇이 중요한지 고르는 데만 쓰고, 그 자체를 사실처럼 서술하지 않습니다.
...
- 길이는 1200~1800자입니다. 이 범위를 넘기지 마세요. 지금까지 자주 3000자를 넘겨서 핵심이 묻혔습니다. 아는 것을 다 쓰는 글이 아니라, 한 가지를 분명히 남기는 글입니다.
- 섹션(h2)은 다섯 개 이하로 만듭니다. 넣을지 말지 망설여지는 문단은 넣지 않습니다.
- 표는 세 열 이하로 만들고, 한 칸은 스무 자를 넘기지 않습니다. 한 칸에 여러 전공이나 여러 조건을 몰아넣으면 표가 아니라 문단입니다. 그렇게 될 것 같으면 표를 쓰지 말고 문장으로 씁니다.
- 학교와 날짜를 줄줄이 나열하지 않습니다. 읽는 사람이 그 목록에서 자기 학교를 찾아내게 만들면 안 됩니다. 나열이 길어지면 표로 옮기고, 문장에는 그중 무엇이 왜 중요한지만 남깁니다.
- 담백한 존댓말. 감탄사와 과장된 수식어를 쓰지 않습니다.
 ...
 * 이 글이 다룰 전공 하나.
 *
 * 요강에는 보컬·기악·작곡·컴퓨터음악이 한 칸에 같이 적혀 있다. 그걸 그대로
 * 옮기면 보컬 지망생이 읽는 글에 음향제작 조건이 섞여 들어간다. 읽는 사람은
 * 자기 것을 골라 읽어야 하고, 잘못 읽으면 엉뚱한 준비를 한다.
 *
 * 그래서 글마다 전공을 하나로 고정한다. 지원자 수가 많은 순으로 자주 나오게
 * 하되 매번 같은 전공만 나오지 않도록 뽑는다.
 */
const MAJORS: { name: string; weight: number; hint: string }[] = [
  { name: '보컬', weight: 5, hint: '가창. 자유곡·지정곡, 반주 형태, 가사 언어 조건이 핵심입니다.' },
  { name: '기악', weight: 3, hint: '연주. 악기별 조건, 초견·반주 유무, 지참 악기 규정이 핵심입니다.' },
  { name: '작·편곡과 싱어송라이터', weight: 3, hint: '자작곡 제출과 시연, 악기 연주 병행 여부가 핵심입니다.' },
  { name: '컴퓨터음악·미디', weight: 2, hint: '포트폴리오 제출 형식, 파일 규격, 질의응답이 핵심입니다.' },
  { name: 'K-POP·실용무용', weight: 2, hint: '가창과 안무를 함께 보는 전형이 많습니다. 곡 수와 시간 제한이 핵심입니다.' },
];

function pickMajor(): { name: string; hint: string } {
  const total = MAJORS.reduce((sum, m) => sum + m.weight, 0);
...
export async function generateBlogDraft(): Promise<DraftResult> {
  ...
  // 예고 글은 전공을 나누지 않는다. 중학생 대상이라 전공 이전의 이야기가 먼저다.
  const forHighSchool = topic.kind === 'guide' && isArtsHighSchool(topic.guide.university);
  const major = forHighSchool ? null : pickMajor();
...
${
  major
    ? `
## 이번 글이 다루는 전공: ${major.name}
${major.hint}

이 글은 ${major.name} 지원자 한 사람을 위해 씁니다.
- 요강에서 ${major.name}에 해당하는 항목만 인용합니다. 다른 전공(예: 보컬 글에 컴퓨터음악, 기악 글에 음향제작) 조건은 본문에도 표에도 넣지 마세요. 한 칸에 여러 전공을 몰아넣는 표는 만들지 않습니다.
- 이 전공 정보가 자료에 없는 학교는 아예 다루지 마세요. 억지로 채우지 않습니다.
- 제목에도 전공이 드러나야 합니다.`
    : ''
}
